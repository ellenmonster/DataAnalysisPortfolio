...
```{r checkoutcome}
glimpse(data_raw$gg2c4)
unique(data_raw$gg2c4) # Only "Yes" and ""
table(data_raw$gg2c4) #716 blanks, 306 "Yes"
```
...
dim(d) # 1022   18
glimpse(d) # OBYear is character class, seems weird
```
...
sum(is.na(d)) # there are 214 missing values
vis_miss(d, sort_miss = TRUE, show_perc = TRUE) 
```
...
d <- d[complete.cases(d), ] # Now 858 records left
```
...
```{r adjust}
glimpse(d)
unique(d$OBYear) # This is the weird one because it is character, but should be integer
d <- d %>%
  filter(OBYear != "0")
unique(d$OBYear)
d$OBYear <- as.integer(d$OBYear)
d <- d %>%
  dplyr::mutate_if(is.character, as.factor)
glimpse(d)
d <- droplevels(d)
(d$MeanD1) # I'm not sure why this is class dbl. Looks like all the numbers are integers. When there is a decimal, it's a zero after the decimal. But also seems odd that the means are always integers. I'll leave it as class dbl. Same for MedianD1.
```
...
d <- droplevels(d)
table(d$Hemisphere)
table(d$Action1) # There is only one "No"
table(d$Path1)
...
(c("Sub Restaruant", "Resaurant", levels(d$Setting_1)[grep("restaurant", levels(d$Setting_1), ignore.case = TRUE)])) # These are the 12 categories that can be considered "Restaurant"
...
d2$Setting_1 <- factor(d2$Setting_1)
levels(d2$Setting_1)

d2$Vomit <- factor(d2$Vomit)
d2 <- droplevels(d2)
```
...
(numeric_var <- select_if(d2, is.numeric) %>% names()) # These are the numeric variables
ridge_list <- list()
...
```{r plots-2}
(categ_var <- select_if(d2, is.factor) %>% names())
...
dim(d4) #Yes! 850 obs and 18 variables
...
parallel::detectCores() # I have 4 cores, so will use all
ncpu=4;
...
sampling_choice = makeResampleDesc("RepCV", reps = 5, folds = 5) # this is from package 'mlr'. "RepCV" is repeated cross-validation.
```
...
(accur <- mean(outcome == predict_null)) # Can manually calculate it this way...accuracy is 0.69
#the null model always predicts "No". In the actual data, we have 591 'No' and 259 'Yes'
```
...
measureAUC(predict_null_resp, outcome, negative = "No", positive = "Yes")
...
kable(unifmat_auc) # Better performance means larger AUC. Best one is Trans1
```
